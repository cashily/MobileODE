...
class CustomActivation(nn.Module):
    def __init__(self,num_layers, epsilon=1):
        ...
        self.epsilon = epsilon/num_layers #0.01
        self.relu6 = nn.ReLU6()  # 实例化 ReLU6

    def forward(self, x):
        # return self.epsilon + (1 - self.epsilon) * self.relu6(x)  # 使用实例化的 ReLU6
        return torch.maximum(torch.tensor(self.epsilon, device=x.device), x)
...
class AdaptiveDiscretizedNeuralODE8_Spatial_pointwise_view_17(nn.Module):
    def __init__(self, i,in_channels, out_channels,num_layers=30):
        ...
        torch.nn.init.normal_(self.delta_t, mean=0.4, std=0.005)  # 初始化 delta_t
...
class InvertedResidual_ode(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #model.layer_3[7].block.red_1x1[0].delta_t
        block.add_module(
            name="red_1x1",
            module=nn.Sequential(AdaptiveDiscretizedNeuralODE8_Spatial_pointwise_view_17(o, hidden_dim, out_channels,num_layers),
