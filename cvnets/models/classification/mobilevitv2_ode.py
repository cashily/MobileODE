...
from cvnets.models.classification.config.mobilevitv2_ode import get_configuration
from cvnets.modules import InvertedResidual, MobileViTBlock,InvertedResidual_ode
from utils import logger

@MODEL_REGISTRY.register(name="mobilevitv2_ode", type="classification")
class MobileViT(BaseImageEncoder):
    ...
    def __init__(self, opts, *args, **kwargs) -> None:
        ...
        image_size//=4
        in_channels = out_channels
        self.layer_2, out_channels = self._make_ode_layer(image_size,image_size,30,
            opts=opts, input_channel=in_channels, mv1_config=mobilevit_config["layer2"]
        )
        self.model_conf_dict["layer2"] = {"in": in_channels, "out": out_channels}
        image_size//=2
        in_channels = out_channels
        self.layer_3, out_channels = self._make_ode_layer(image_size,image_size,20,
            opts=opts, input_channel=in_channels, mv1_config=mobilevit_config["layer3"]
        )
        self.model_conf_dict["layer3"] = {"in": in_channels, "out": out_channels}
        image_size//=2
        in_channels = out_channels
        self.layer_4, out_channels = self._make_ode_layer(image_size,image_size,20,
            opts=opts,
            input_channel=in_channels,
            mv1_config=mobilevit_config["layer4"],
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        group.add_argument(
            "--model.classification.mitv2.mode",
            type=str,
            default="small",
        ...
        group.add_argument(
            "--model.classification.mitv2.transformer-norm-layer",
            type=str,
            default="layer_norm",
            help="Normalization layer in transformer. Defaults to LayerNorm",
        )
        group.add_argument(
            "--model.classification.mitv2.no-fuse-local-global-features",
            action="store_true",
            help="Do not combine local and global features in MobileViT block",
        )
        group.add_argument(
            "--model.classification.mitv2.conv-kernel-size",
            type=int,
            default=3,
        ...
        group.add_argument(
            "--model.classification.mitv2.head-dim",
            type=int,
            default=None,
            help="Head dimension in transformer",
        )
        group.add_argument(
            "--model.classification.mitv2.number-heads",
            type=int,
            default=None,
    ...
    def _make_ode_layer(
        ...
        **kwargs
        ...
        expand_ratio = mv1_config.get("expand_ratio", 4)
        ...
        **kwargs
        ...
        block.append(
            MobileViTBlock(
                ...
                dropout=getattr(opts, "model.classification.mitv2.dropout", 0.1),
                ffn_dropout=getattr(opts, "model.classification.mitv2.ffn_dropout", 0.0),
                attn_dropout=getattr(
                    opts, "model.classification.mitv2.attn_dropout", 0.1
                ),
                head_dim=head_dim,
                no_fusion=getattr(
                    opts,
                    "model.classification.mitv2.no_fuse_local_global_features",
                    False,
                ),
                conv_ksize=getattr(
                    opts, "model.classification.mitv2.conv_kernel_size", 3
                ),
            )
        )
