...
def load_pth(load_dict,net):
    # pdb.set_trace()
    checkpoint = torch.load(load_dict, map_location='cpu')
    print("Load pre-trained checkpoint from: %s" % load_dict)
    cur_model_dict = net.state_dict()
    if 'model' not in checkpoint.keys():
        n = 0
        for key, value in checkpoint.items():
            if key in cur_model_dict.keys() and cur_model_dict[key].shape == value.shape:
                n+=1
                cur_model_dict[key] = value
        net.load_state_dict(cur_model_dict)
        print("mae_no_model has done,total {} key.".format(n))
